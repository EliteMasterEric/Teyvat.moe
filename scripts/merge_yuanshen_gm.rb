...
def merge_marker(input_migrated, input_yuanshen)
  if input_migrated.nil? then
    ...
    return {
      ...
      'importIds' => input_migrated.fetch('importIds', {}).merge(input_yuanshen['importIds']),
      "popupTitle" => input_migrated.fetch('popupTitle', {}).merge(input_yuanshen.fetch('popupTitle', {})),
      "popupContent" => input_migrated.fetch('popupContent', {}).merge(input_yuanshen.fetch('popupContent', {})),
...
def merge_data(input_migrated, input_yuanshen)
  ...
  yuanshen_markers = input_yuanshen['data'].map {|x| [x['importIds']['yuanshen'][0], x]}.to_h
  
  yuanshen_used = []

  output_json['data'] = migrated_markers.map do |key, migrated|
    if (migrated.fetch('importIds', {})['yuanshen']) then
      yuanshen = yuanshen_markers[migrated['importIds']['yuanshen'][0]]
      if (yuanshen != nil) then
        yuanshen_used.concat(migrated['importIds']['yuanshen'])
        next merge_marker(migrated, yuanshen)
      end
    end
    next migrated
  end

  yuanshen_markers_unused = yuanshen_markers.reject {|key, value| 
    yuanshen_used.include?(key)
  }

  output_json['data'].concat(yuanshen_markers_unused.values.map{|value| 
    value['popupTitle']['en'] = "NEW YUANSHEN MARKER: #{value['popupTitle']['en']}"
    value
  })
