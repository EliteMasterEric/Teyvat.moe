...
def migrate_marker(input_json, absolute_name)
  output_json = deep_dup(input_json)

  # Move coordinates.
  output_json['coordinates'] = reproject_point(input_json['coordinates']).map {|i| truncate_number(i)}
...
def migrate_feature(input_json, absolute_name)
  # Duplicate the default feature, and merge our data in.
  output_json = deep_dup(input_json)

  output_json['data'] = input_json['data'].map { |x| migrate_marker(x, absolute_name) }
...
def process_data_file(input, output)
  ...
  output_json = migrate_feature(input_json, absolute_name)
