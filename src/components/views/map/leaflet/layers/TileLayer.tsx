...
const ErrorTileLayer: (callback: (error: Error | null) => void) => ErrorHandlerComponent = (
  onLoadError
) => ({ error }) => {
...
interface MainTileLayerProps {
  onLoadStart: LeafletEventHandlerFn;
  onLoadError: (error: Error | null) => void;
  onLoadSuccess: LeafletEventHandlerFn;
}

const MainTileLayer: FunctionComponent<MainTileLayerProps> = ({
  onLoadStart,
  onLoadError,
  onLoadSuccess,
}) => {
  return (
    <ErrorHandler errorHandler={ErrorTileLayer(onLoadError)}>
