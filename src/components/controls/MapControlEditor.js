  ...
  return (
    <div className={clsx('map-controls-editor-element')}>
      ...
      <div className={clsx('map-controls-editor-element-row')}>
        ...
        <input
          value={marker?.properties?.popupTitle?.en}
          onChange={updateMarkerProp('popupTitle.en')}
        />
      </div>
      <div className={clsx('map-controls-editor-element-row')}>
        <span className={clsx('map-controls-editor-element-label')}>Content</span>
        <textarea
          value={marker?.properties?.popupContent?.en}
          onChange={updateMarkerProp('popupContent.en')}
        />
      </div>
      <div className={clsx('map-controls-editor-element-row')}>
...
const MapControlEditor = ({ mapPreferences, setMapPreferences, setTab }) => {
  ...
  const updateElementProperties = (index, key, value) => {
    setElementData((old) => {
      ...
      _.set(newElementData[index], `properties.${key}`, value);
  ...
  const submitEditorData = (data) => {
    const jsonData = generatePrettyJSON({
      ...data,
      data: mapPreferences?.editor?.feature?.data,
    });
