...
import { ObjectHashError } from '~/error/ObjectHashError';
 ...
 */
export const hashObject = (input: any, options?: any): string => {
  try {
    const fullOptions = {
      algorithm: 'sha1',
    ...
    console.error(input);
    throw new ObjectHashError(input);
  }
};
 ...
 */
export const openURLInWindow = (url: string): null => {
...
export const useDebouncedState = <T extends unknown>(
  ...
  const onStateChangedRef = React.useRef<(newValue: T) => void>(null);
  ...
  const onStateChangedDebounced: _.DebouncedFunc<(newValue: T) => void> = React.useMemo(
 ...
 */
export const CloneProps = ({ children, ...other }): ReactElement => children(other);
